...
import { showingTranslateValue } from "utils/translate";
import SettingServices from "services/SettingServices";
import { Badge, Button, ModalBody, Pagination, Table, TableCell, TableContainer, TableFooter, TableHeader } from "@windmill/react-ui";
...
const ProductDetails = ({ initialRatings }) => {
  const { id } = useParams();
  const { t } = useTranslation();
  const { handleUpdate } = useToggleDrawer();
  const { attribue } = useProductSubmit(id);
  ...
  const [isOrderProcessVisible, setIsOrderProcessVisible] = useState(false);
  const [defaultMethodDetails, setDefaultMethodDetails] = useState("");
  const [submitted, setSubmitted] = useState(false);
  console.log("hi", data)

  const { ratings, setRatings, submitRating, updateRating, deleteRating } = useProductSubmit(id, initialRatings);
  ...
  const handleSubmitRating = async (reviewData) => {
    console.log("sents", reviewData);
    console.log("sents", adminInfo);
    const userId = adminInfo?._id;
    try {
      const reviewWithUserId = { ...reviewData, user: userId };
      await submitRating(reviewWithUserId);
      setSubmitted(true);
    } catch (error) {
  ...
  // const handleSubmitRating = async (reviewData) => {
  //   const userId = adminInfo?._id;
  //   if (data?.reviews?.some(review => review?.user === userId)) {
  //     return;
  //   }
  //   try {
  //     const reviewWithUserId = { ...reviewData, user: userId };
  ...
  //   if (userRating > 0) {
  //     updateRating(ratingId, userRating);
  //     setUserRating(0);
  //   }
  // };
  ...
  return (
    <>
      ...
      {/* )} */}
      {/* {data?.isCombination && data?.variants?.length > 0 && !loading && ( */}
      {/* Add section to display average rating */}
      {/* Add section to display reviews */}
      {/* <div className="p-5 md:p-8">
        <h2 className="text-lg font-semibold mb-3">{t("ProductReviews")}</h2>
        {reviews.map((review, index) => (
          <div key={index} className="mb-4">
            <p className="font-semibold">{t("Rating")}: {review.rating}</p>
            <p>{t("Comment")}: {review.comment}</p>
          </div>
        ))}
      </div> */}
      ...
      <ReviewModal
        ...
        onSubmit={(reviewData) => {
          handleSubmitRating(reviewData);
        }}
      />
      <button onClick={handleToggleReviewModal}>{t("Write a Review")}</button>

      {data?.reviews?.length > 0 && (
        // <FeedbackMonitor productId={id} reviews={data?.reviews} ratings={data?.averageRating} />
        <FeedbackMonitor
          productId={id}
      ...
      {/* <div className="p-5 md:p-8">
        <button onClick={handleTogglePaymentModal}>{t("Open Payment Methods")}</button>
      </div>
      <PaymentMethods
      ...
      <div>
        <div>
          <button onClick={handleOpenOrderModal}>{t("BuyNow")}</button>
        </div>
        <Modal open={isOrderModalOpen} onClose={handleCloseOrderModal}>
          <ModalBody className="text-center custom-modal px-8 pt-6 pb-4">
            <OrderProcessModal onSubmitOrder={handleSubmitOrder} id={id} selectedPaymentMethod={defaultMethodDetails} />
          </ModalBody>
        </Modal>
        {isOrderProcessVisible && <OrderProcessModal onClose={() => setIsOrderProcessVisible(false)} id={id} />}
      </div>
      {data?.isCombination && variantTitle?.length > 0 && !loading && (
        <>
          <PageTitle>{t("ProductVariantList")}</PageTitle>
